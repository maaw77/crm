...
async def main():
    ...
    async with asyncpg.create_pool(host=con_par.HOST_DB,
        ...
        async with aiohttp.ClientSession(headers=web.HEADERS, cookie_jar=jar) as session:
            ...
            #                 (fileapi.fetch_data_file, {'name': fileapi.FILE_NAMES[7]},
            #                  database.insert_tank_table, TankTable,),
            #                 (fileapi.fetch_data_file, {'name': fileapi.FILE_NAMES[6]},
            #                  database.insert_sheet_table, SheetTable,),
            #                 (fileapi.fetch_data_file, {'name': fileapi.FILE_NAMES[0]},
            #                  database.insert_azs_table, AZSTable,),
            #                 (fileapi.fetch_data_file, {'name': fileapi.FILE_NAMES[2]},
            ...
            vldrs_ftchrs = ((web.fetch_table, {'url': web.webapi.URL_GSM_TABLE, 'session': session},
                             database.insert_gsm_table, GsmTable,),
                            (web.fetch_table, {'url': web.webapi.URL_TANK_TABLE, 'session': session},
                             database.insert_tank_table, TankTable,),
                            (web.fetch_table, {'url': web.webapi.URL_SHEET_TABLE, 'session': session},
                             database.insert_sheet_table, SheetTable,),
                            (web.fetch_table, {'url': web.webapi.URL_AZS_TABLE, 'session': session},
                             database.insert_azs_table, AZSTable,),
                            (web.fetch_table, {'url': web.webapi.URL_EXCHANGE_TABLE, 'session': session},
                             database.insert_exchange_table, ExchangeTable,),
                            (web.fetch_table, {'url': web.webapi.URL_REMAINS_TABLE, 'session': session},
                             database.insert_remains_table, RemainsTable,),)
            ...
            tasks = [asyncio.create_task(bot_table(session,
                                                   ...
                                                   argv_fetcher=vf[1],
                                                   insert_table=vf[2],
                                                   validator=vf[3])) for vf in vldrs_ftchrs]
